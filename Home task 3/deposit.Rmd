...
lines(c(0,1), c(0.10,0.10), col="red", lwd=2)

data2 <- data[cooks.distance(m7)<0.10,]
...
При пороге 0.79 построенная модель обеспечивает чувствительность и специфичность, равные ≈ 0.88; площадь под ROC-кривой составляет 0.9433456. Значимость модели по критерию отношения правдоподобия равна `r lrtest(m8,m0)$"Pr(>Chisq)"[2]`.
