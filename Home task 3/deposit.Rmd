...
m8  <- glm(y ~  duration + campaign + emp.var.rate + cons.price.idx + cons.conf.idx + poutcome + month + contact  + month:duration , family=binomial(), data=data2)
summary(m8)
```
...
plot(r)
```
При пороге 0.78 построенная модель обеспечивает чувствительность и специфичность, равные ≈ 0.9; площадь под ROC-кривой составляет 0.943551. Значимость модели по критерию отношения правдоподобия равна `r lrtest(m7,m0)$"Pr(>Chisq)"[2]`.

Вывод:

- В декабре и марте клиенты склонны брать кредит.
- В августе и сентябре на готовность клиента открыть кредит влияет продолжительность последнего контакта с клиентом.
- Для успешности камании следует проводит больше обзвонов
- Темп изменения занятости,  consumer confidence index,  consumer price index оказывают влияние на готовность взять кредит.
- Звонки на стационарный телефон явлюется эффективными.
- В ноябре обзвон будет успешней, если ранее не проводились подобные кампании.
