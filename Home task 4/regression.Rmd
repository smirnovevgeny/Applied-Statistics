...
```{r, echo=TRUE, fig.height=5.5, fig.width=10}
par(mfrow=c(1,2))
acf(diff(tSeries, 12), lag.max=5*12, main="")
pacf(diff(tSeries, 12), lag.max=5*12, main="")
```

Будем искать модель в окрестности ARIMA(1,0,1)(2,1,1)$_{12}$

Модель                     | AICc
-------------------------- | ------------
ARIMA(1,0,1)(2,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,0,1), seasonal=c(2,1,1), lambda=LambdaOpt)$aicc`
ARIMA(0,0,1)(2,1,1)$_{12}$ | `r Arima(tSeries, order=c(0,0,1), seasonal=c(2,1,1), lambda=LambdaOpt)$aicc`
ARIMA(1,0,0)(2,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,0,0), seasonal=c(2,1,1), lambda=LambdaOpt)$aicc`
ARIMA(1,0,1)(2,1,0)$_{12}$ | `r Arima(tSeries, order=c(1,0,1), seasonal=c(2,1,0), lambda=LambdaOpt)$aicc`
ARIMA(1,0,1)(1,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,0,1), seasonal=c(1,1,1), lambda=LambdaOpt)$aicc`
ARIMA(1,0,2)(2,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,0,2), seasonal=c(2,1,1), lambda=LambdaOpt)$aicc`
ARIMA(1,0,1)(2,1,2)$_{12}$ | `r Arima(tSeries, order=c(1,0,1), seasonal=c(2,1,2), lambda=LambdaOpt)$aicc`
ARIMA(1,0,1)(3,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,0,1), seasonal=c(3,1,1), lambda=LambdaOpt)$aicc`
ARIMA(0,0,0)(2,1,1)$_{12}$ | `r Arima(tSeries, order=c(0,0,0), seasonal=c(2,1,1), lambda=LambdaOpt)$aicc`
ARIMA(1,0,1)(1,1,0)$_{12}$ | `r Arima(tSeries, order=c(1,0,1), seasonal=c(1,1,0), lambda=LambdaOpt)$aicc`
ARIMA(1,0,0)(1,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,0,0), seasonal=c(1,1,1), lambda=LambdaOpt)$aicc`
ARIMA(0,0,1)(1,1,1)$_{12}$ | `r Arima(tSeries, order=c(0,0,1), seasonal=c(1,1,1), lambda=LambdaOpt)$aicc`
ARIMA(1,0,1)(0,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,0,1), seasonal=c(0,1,1), lambda=LambdaOpt)$aicc`
ARIMA(2,0,1)(0,1,1)$_{12}$ | `r Arima(tSeries, order=c(2,0,1), seasonal=c(0,1,0), lambda=LambdaOpt)$aicc`
ARIMA(1,0,2)(0,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,0,2), seasonal=c(0,1,1), lambda=LambdaOpt)$aicc`
ARIMA(1,0,1)(0,1,2)$_{12}$ | `r Arima(tSeries, order=c(1,0,1), seasonal=c(0,1,2), lambda=LambdaOpt)$aicc`

Возьмём модель ARIMA(1,0,1)(0,1,1)$_{12}$, как модель с наименьшим AIC.

```{r, echo=TRUE, fig.height=4.5, fig.width=10}
fit <- Arima(tSeries, order=c(1,0,1), seasonal=c(0,1,1), lambda=LambdaOpt)
res <- residuals(fit)
plot(res)
```
...
Настроив выбранную модель на обучающей выборке, посчитаем её качество на тестовой:
```{r, echo=TRUE, fig.height=5.5, fig.width=10}
fitShort <- Arima(trainSeries, order=c(1,0,1), seasonal=c(0,1,1), lambda=LambdaOpt)
fc       <- forecast(fitShort, h=D)
accuracy(fc, testSeries)
...
dm.test(res.ets, res)
dm.test(res.ets, res, alternative = "less")
```
Согласно критерию Диболда-Мариано, прогнозы метода ARIMA значимо менее точные, поэтому в качестве финального выберем прогноз найденный ETS.


### Финальный прогноз

```{r, echo=TRUE, fig.height=5.5, fig.width=10}
fl <- forecast(fit.ets, h=D)
print(fl)
