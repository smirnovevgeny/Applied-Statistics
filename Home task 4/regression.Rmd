...
- Сравненить и выбрать лучшую модель по критерию Диболда-Мариано;
...
Будем искать модель в окрестности ARIMA(1,1,1)(2,1,1)$_{12}$
...
ARIMA(1,1,2)(1,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,1,2), seasonal=c(1,1,1), lambda=LambdaOpt)$aicc`
ARIMA(1,1,1)(1,1,2)$_{12}$ | `r Arima(tSeries, order=c(1,1,1), seasonal=c(1,1,2), lambda=LambdaOpt)$aicc`
ARIMA(1,1,1)(2,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,1,1), seasonal=c(2,1,1), lambda=LambdaOpt)$aicc`
ARIMA(0,1,0)(1,1,1)$_{12}$ | `r Arima(tSeries, order=c(0,1,0), seasonal=c(1,1,1), lambda=LambdaOpt)$aicc`
ARIMA(1,1,1)(0,1,0)$_{12}$ | `r Arima(tSeries, order=c(1,1,1), seasonal=c(0,1,0), lambda=LambdaOpt)$aicc`
ARIMA(1,1,0)(0,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,1,0), seasonal=c(0,1,1), lambda=LambdaOpt)$aicc`
ARIMA(0,1,1)(0,1,1)$_{12}$ | `r Arima(tSeries, order=c(0,1,1), seasonal=c(0,1,1), lambda=LambdaOpt)$aicc`
ARIMA(1,1,1)(0,1,1)$_{12}$ | `r Arima(tSeries, order=c(1,1,1), seasonal=c(0,1,1), lambda=LambdaOpt)$aicc`
ARIMA(2,1,1)(0,1,0)$_{12}$ | `r Arima(tSeries, order=c(2,1,1), seasonal=c(0,1,0), lambda=LambdaOpt)$aicc`
...
Возьмём модель ARIMA(0,1,1)(0,1,1)$_{12}$, как модель с наименьшим AIC.

```{r, echo=TRUE, fig.height=4.5, fig.width=10}
fit <- Arima(tSeries, order=c(0,1,1), seasonal=c(0,1,1), lambda=LambdaOpt)
res <- residuals(fit)
plot(res)
```
...
Настроив выбранную модель на обучающей выборке, посчитаем её качество на тестовой:
```{r, echo=TRUE, fig.height=5.5, fig.width=10}
fitShort <- Arima(trainSeries, order=c(0,1,1), seasonal=c(0,1,1), lambda=LambdaOpt)
fc       <- forecast(fitShort, h=D)
accuracy(fc, testSeries)
...
fit.auto
```
Предлагается модель ARIMA(1,1,1,0)(2,0,0)$_{12}$. Её AICc выше, чем у модели, подобранной вручную. Посмотрим на её остатки:
```{r, echo=TRUE, fig.height=4.5, fig.width=10}
